...
def log_time(tag, start):
    ...
    print(f'{tag} Time: {end - start:.3f}초')
    return end
...
step_time = log_time("CSV Read", script_start)

# sentence-transformer 내장 모델
model = SentenceTransformer('distiluse-base-multilingual-cased-v2')
step_time = log_time("Model Load", step_time)

# 사용자가 입력한 스크립트를 text 변수에 저장
# 내용이 없으면 exit()
if len(sys.argv) < 2:
...
text = sys.argv[1]
step_time = log_time("Input Save", step_time)

# 입력한 텍스트를 인코딩한다.
x = model.encode(df['story'])
y = model.encode(text)
step_time = log_time("Encoding", step_time)

# 유사도 계산
cos_scores = util.pytorch_cos_sim(x, y).squeeze()
cos_scores = np.array(cos_scores)
step_time = log_time("Similarity Calculation", step_time)
